...
| --- | --- | --- | --- | --- |
| Sage 10.9 全量 API 索引读取 | 外部索引 | 外部索引 | **已验证（开发索引/query）**：84,385 normalized identities、84,491 raw AST declarations、2,843 source files、conflicts=0；当前 staging 目录未提供 envelope/receipt，因此 product-sidecar 验证仍未完成 | core/sage-api query；SageIntelligenceHarnessTest.testConfiguredFullIndexCoverageMatrix |
...
| 全量返回合同质量审计 | 外部索引 | 外部索引 | **已验证（只读审计）**：52,723 callable entries、52,299 signatures；UNKNOWN 30,895、DYNAMIC 16、TYPE_VARIABLE 361、CONCRETE 3,826。相对历史 45,126 UNKNOWN 累计减少 14,231（31.54%）；本轮相对 32,536 再减少 1,641。审计另列出宽泛内建/结构化基类，绝不把它们计作具体 Sage 返回；本轮追加 Python 协议、自描述条件输出重载、CTF 位向量/二进制串/排列、尺寸、S-box 列表、sub-key、Mini-AES 矩阵父对象 TypeVar、SR/AES、经典密码体制、LFSR 和 BooleanFunction 合同 | `tools/sage-api-index/audit_contracts.py`；`sage-api-curated-type-contracts.audit.next.json` |
