...
- **可定制的 Sage 后缀模板集** —— **设置 → 编辑器 → 后缀补全** 中 Sage
  模板挂在独立顶级分组 `SageMathPostfix` 下，带语法高亮的 before/after
  预览与中文描述；内建模板可双击改名，「+」可新建 Sage 模板（条件列表为
  对 sage 判断准确的集合：字符串 / list / dict / set / tuple / 非 None +
  现成 `sage.all` 类条件），新建模板重启后仍在、只在 `.sage` 文件生效
...
插件的 Sage 词法器与预处理器语义逐一对齐：`^` 按幂、`^=` 按幂赋值、
`^^` 按按位异或、`^^=` 按异或赋值解析——`e^254` 因此走 `__pow__` 类型链、
四种写法都不再报无中生有的语法错误，而屏幕上看到的文本永远不变。
