...
- 本轮 coverage/expected 红测首次执行 `python -m unittest tools/sage-api-index/test_import_wsl_artifact.py` 真实 exit code `1`：22 tests 中 4 errors；新增 expected contract 尚未实现，空 expected coverage 的 mock fixture 未接受 subprocess 关键字参数，且 returncode=0 缺少 coverage 的旧逻辑直接抛 FileNotFoundError、未持久化 failed receipt。已记录后修复测试 fixture并实现 fail-closed contract、UNSCOPED/SCOPED coverage 与 post-generator terminal failure。
- coverage helpers 接入后复跑同一测试真实 exit code `1`：22 tests 中 10 errors，所有 probe/live/replay 路径暴露 `NameError: probe_digest is not defined`；新增 helper 替换时遗漏原有 probe_digest 定义。已记录后恢复 probe_digest，再继续修复后置 receipt 状态与 expected contract。
- 恢复 probe_digest 后复跑同一测试真实 exit code `1`：22 tests 中 3 errors；新增 coverage 红测暴露 receipt 尚未写入 coverage 摘要，returncode=0 缺少 coverage 仍直接抛 FileNotFoundError、没有 failed 终态。已记录后补充 coverage summary 写 receipt，并统一后置异常持久化 failed。
- 完成本轮实现后，`python -m unittest tools/sage-api-index/test_import_wsl_artifact.py tools/sage-api-index/test_generate.py` 真实 exit code `0`（47 tests），`py_compile` 4 个 Python 模块真实 exit code `0`，`:core:sage-api:test -PrunSageApiTests=true --no-daemon --console=plain` 真实 exit code `0`（BUILD SUCCESSFUL），`git diff --check` 真实 exit code `0`。新增 expected contract/coverage receipt 与 replay 免责声明仍未提交。
- 第一次默认 LIVE full（无 `--probe-json`/无 `--allow-probe-replay`）输出 `build/sage-api-real/live-1`，真实 exit code `0`：Sage 10.9、Python 3.13.15、stubgen 0.8.3、probeDigest=44945e3f...、treeDigest=69d0524a...、stubFileCount=2839、generated/discovered=2837/2837、expectedCount=6、coverage=SCOPED ratio=1.0、covered=6、missing=0、conflicts=5、isComplete=false。
- 第二次独立默认 LIVE full 输出 `build/sage-api-real/live-2`，真实 exit code `0`，上述 probe/tree/file/report/expected/coverage/conflict 摘要相同；两次均没有读取 checkpoint，默认 LIVE full 的这些 deterministic fields 重复。
- 显式 replay scoped import 输出 `build/sage-api-real/replay-scoped`，真实 exit code `0`，probeMode=REPLAY、probeDigest=44945e3f...、expectedDigest=58be9132...、expectedCount=6、coverage.scope=SCOPED、ratio=1.0、conflictCount=5。REPLAY 仍仅本机恢复/调试证据，probeDigest 非签名或 release attestation。
