...
- JetBrains 插件商城：已备好发布要素（plugin.xml change-notes + gradle `publishing.token=PUBLISH_TOKEN`）。**首次上传必须由用户账号在网页手动完成**（https://plugins.jetbrains.com/plugin/upload，上传 `build/distributions/sage-ide-support-1.4.1.zip`，填 **License=GPL-3.0**、Repository URL、Category、Tags；token 无法创建全新插件，报错 "Cannot find plugin...upload at least once manually"）。用户已提供发布 token（仅存在于用户 JetBrains 账号与本次会话，**严禁写入仓库**）。插件页上线后，后续版本 `$env:PUBLISH_TOKEN=...; gradle publishPlugin` 一条命令即可，审核约 2 个工作日。发布前记得 plugin.xml 版本号随 release bump。**许可证 2026-08-17 已从 MIT 换成 GPL-3.0（两仓库统一，用户拍板）**：注意 PyPI 已发布的 ≤0.8.0 与 GitHub 已发布的 ≤v1.4.1 在法律上仍是 MIT，新版本才适用 GPL。**PEP 639 坑**：pyproject 用 SPDX `license = "GPL-3.0-only"` 时**不得**同时保留 `License ::` classifier（setuptools≥77 直接 InvalidConfigError，CI 的 `pip install -e .` 在跑测试前就炸；本地 PYTHONPATH 跑 unittest 不经过打包所以假绿）——修复 `864a445` 删 classifier。**教训：改打包元数据后必须本地 `python -m build` + `pip install -e .` 验证，不能只跑 unittest。**
