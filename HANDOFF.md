...
| ④ Sage 上游 PR #42670 | `G:\Projects\sage-fork` | **2026-08-18 新评审已处理（head `c5bdfdf`）**：vincentmacri（CHANGES_REQUESTED）建议把转发式 `def __call__` 覆写换成**类体裸注解** `__call__: Callable[..., FiniteField_base]`（nbruin 附议：为类型推断增加每次调用开销不划算）→ 已按建议改：裸注解**无赋值** → `type(GF).__call__` 仍是继承的 `UniqueFactory.__call__`（零每次调用开销，仅类创建时多一条 `__annotations__`）；测试移入类级 TESTS 块 + 新增注解结构化断言（全 `==` 无 `is`，3.14 安全）；**cxzhong 的别名要求一并落地**（`FiniteField as FiniteField_base`——PEP 649 下 3.14 惰性求值走活模块命名空间，裸名会解析到工厂实例，CPython 3.14.4 已复现）。验证：mypy + pyright 1.1.332（`types.yml` 钉版）/1.1.411 全推断正确、两文件 overlay 到 10.9 真机跑 TESTS 全绿、ruff CI 配置 `--no-preview`/`--preview` 双通过。已发评论 issuecomment-5318318884（**引用 stubgen 与 JetBrains 插件项目说明静态类型层价值**，回应 nbruin「类型推断无实益」）。推送重生成 workflow run，待维护者批准执行 + 复审。ecl.pyx 崩溃已提 issue #42680。 |
...
**后续动作（2026-08-18 更新）**：
- 上游 PR #42670：**新评审（vincentmacri CHANGES_REQUESTED + nbruin 附议）已按建议处理并推送 `c5bdfdf`**——转发式 `__call__` 覆写改为类体裸注解（零每次调用开销），测试移类级 TESTS 块，cxzhong 别名要求落地；已回评论 issuecomment-5318318884（引用 stubgen + JetBrains 插件项目论证静态类型价值）。待维护者批准新 workflow run + 复审。**注意**：现在「勿自行推新 commit」的红线已解除（新评审要求改代码），但下一步仍等维护者批复，不要自行追加提交。
...
> 继续 G:\Projects\sage-ide-support 插件工作。先读 HANDOFF.md 全文（重点是「v1.6.0」四轮 bugfix、「v1.6.1 popup 根因与修复」、「v1.6.2 设置页分组+新建模板」、「v1.6.3 四修」、「v1.6.4 条件精准集」、「v1.6.5 description.html 三件套修复」、「v1.6.6 设置页描述中文化」、「已知坑清单」、「独立 Sage 语法树可行性分析」与「.lexer-test」测试设施）。
> 当前状态：**v1.6.6 已正式发布**（用户全清单实测通过 → tag + push → CI 双发布：GitHub Release + 商城）。功能全景：popup（v1.6.1）、设置页分组/新建模板（v1.6.2）、b2i/i2b 展开/预览无点/双击编辑（v1.6.3）、条件精准集（v1.6.4）、预览 description.html 三件套（v1.6.5）、设置页描述中文化（v1.6.6）。红线（永久）：不得回退到复用 PyCallWrapPostfixTemplate（预览会变 list）、不得改回 language="Python"/"Sage" 注册（用 SageMathPostfix meta-language）、不得重新加回 isEditable=false、不得把 Py 的 number/iterable/boolean/exception 条件加回编辑器、不得删除 per-key 的 description.html/before/after 三件套任一资源、设置页描述保持中文。后续候选工作：stubgen 0.8.1 翻译批处理续跑、上游 PR #42670/#42672/#42675 推进、PR #3614 方言语言解析特性请求。
