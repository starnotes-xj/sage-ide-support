...
| ② 插件 | 本仓库 | **v1.6.2**：Sage-only 后缀模板集（Python 自带模板经 base-language 链自动并入）+ 完整 caret 词法重映射（`SageCaretLexer`：`^`→EXP、`^=`→EXPEQ、`^^=`→XOREQ 合并、`^^`→XOR），`e^254` 按 `__pow__` 类型化；**后缀补全 popup 修复**（`SageFile.getFileType()` 覆写仿 PyiFile + `.b2i`/`.i2b` key 补点 + DumbAware）；**设置页**：Sage 模板独立顶级分组（注册于 Python 族 meta-language `SageMathPostfix`）+ 「+」可新建 Sage 后缀模板（PyEditablePostfixTemplate 机制 + SageFile 门）；inspection 改检测「文本为 ^ 的 EXP / 文本为 ^= 的 EXPEQ」；idea-version 放宽 **261–263.***（2026.1–2026.3 全年）；**GitHub Actions CI**（push 构建 zip 上传 artifact，v* tag 自动挂 Release）；SDK 双模式（CI 下载 `pycharm("2026.2.1")` + `bundledPlugin("PythonCore")`，本地用 D:\JetBrains\PyCharm）；CI 已绿 |
...
> 继续 G:\Projects\sage-ide-support 插件工作。先读 HANDOFF.md 全文（重点是「v1.6.0」四轮 bugfix、「v1.6.1 popup 根因与修复」、「v1.6.2 设置页分组+新建模板」、「已知坑清单」、「独立 Sage 语法树可行性分析」与「.lexer-test」测试设施）。
> 当前状态：**popup 已在 v1.6.1 修复（`SageFile.getFileType()` 覆写仿 PyiFile + `.b2i`/`.i2b` key 补点 + DumbAware）；设置页已在 v1.6.2 修复（meta-language `SageMathPostfix` 独立顶级分组 + 可新建/编辑/持久化 Sage 后缀模板）**，全回归通过（.lexer-test 五测试 + buildPlugin + verifyPlugin 双版本 Compatible；TestPostfix 含 meta-language 收集契约验证）。唯一待办：**用户装 `build/distributions/sage-ide-support-1.6.2.zip` 实测**：① 设置页树出现独立 "SageMathPostfix" 顶级组（28 模板，不再挂 Python 下）；② 「+」→ SageMath → 新建 `.foo` 弹窗正常、`m.foo` 在 .sage 弹出展开、.py 不弹；③ 新建模板重启后仍在（存储往返）、删除正常；④ 1.6.1 全清单回归（popup/展开/.py 隔离/预览 + `e^254`/`^^`/糖语句）。若实测仍有问题：按 v1.6.1 节末尾的探针方法出诊断版读 idea.log（确认 `Loaded custom plugins ... (1.6.2)`）。红线：不得回退到复用 PyCallWrapPostfixTemplate（预览会变 list）、不得去掉内建模板 isEditable=false、不得改回 language="Python"/"Sage" 注册（详见对应根因条目）。实测通过后：commit → `git tag v1.6.2` → push（CI 双发布）。
